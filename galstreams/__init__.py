...
def get_random_spherical_coords(
    ...
    phi_s, theta_s = get_random_spherical_angles(
        n, degree=degree, az=az, lat=lat
    )
...
class Footprint:
    def __init__(
        ...
        if "GC" in cootype:
            ...
            if vxyz_gal:
                self.vx, self.vy, self.vz = vxyz_gal
            ...
            if "gal" in cootype:
                if pmlon is not None:
                    if is_pml_star:
                        self.pmlstar, self.pml = (
                            pmlon,
                            pmlon / np.cos(_f * self.b),
                        )
                    else:
                        self.pmlstar, self.pml = (
                            pmlon * np.cos(self._f * self.b),
                            pmlon,
                        )
                if pmlat is not None:
                    self.pmb = pmlat
            else:
                if pmlon is not None:
                    if is_pml_star:
                        self.pmrastar, self.pmra = (
                            pmlon,
                            pmlon / np.cos(_f * self.dec),
                        )
                    else:
                        self.pmrastar, self.pmra = (
                            pmlon * np.cos(self._f * self.dec),
                            pmlon,
                        )
                if pmlat is not None:
                    self.pmdec = pmlat
    ...
    def compute_sky_center(self):
        ...
        mm = bovyc.lbd_to_XYZ(
            self.l, self.b, np.ones_like(self.l), degree=True
        )
        if self.l.size > 1:
            _xx, _yy, _zz = mm.T
        else:
        ...
        if hasattr(self, "phi") and hasattr(self, "theta"):
            ...
            self.cphi, self.ctheta = bovyc.XYZ_to_lbd(
                _xgc, _ygc, _zgc, degree=True
            )[:2]
    ...
    def mask_footprint(self, mask):
        ...
        for myattr in self.__dict__.keys():
            ...
            if (
                not callable(getattr(self, myattr))
                and np.ndim(getattr(self, myattr)) >= 1
            ):
    ...
    def compute_galactocentric_coords(self, verbose=False, degree=True):
        ...
        if verbose:
            print(
                "Converting Heliocentric Galactic Spherical to Heliocentric Cartesian coords..."
            )
        ...
        if verbose:
            print(
                "Converting Heliocentric Cartesian to Galactocentric Cartesian coords..."
            )
        m = bovyc.XYZ_to_galcenrect(
            self.xhel,
            self.yhel,
            self.zhel,
            Xsun=self.xsun,
    ...
    def compute_heliocentric_coords(self, verbose=False, degree=True):
        ...
        xg, yg, zg = bovyc.lbd_to_XYZ(
            self.phi, self.theta, self.Rgal, degree=degree
        ).T
        ...
        self.l, self.b, self.Rhel = bovyc.XYZ_to_lbd(
            self.xhel, self.yhel, self.zhel, degree=degree
        ).T
        ...
        if hasattr(self, "vx") and hasattr(self, "vy") and hasattr(self, "vz"):
            ...
            self.pmrastar, self.pmdec = pmllpmbb_to_pmrapmdec(
                self.pml, self.pmb, self.l, self.b, degree=degree, epoch=2000.0
            )
...
class MWStreams(dict):
    ...
    def init_by_end_points(self, gcstep=0.01, verbose=False):
        ...
        lib_end_points_filen = os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            "lib",
            "lib_by_pair.dat",
        )

        lono, lato, lonf, latf, ro, rf, width = scipy.genfromtxt(
            lib_end_points_filen,
            usecols=(3 - 1, 4 - 1, 5 - 1, 6 - 1, 8 - 1, 9 - 1, 10 - 1),
            unpack=True,
        )
        name, sname, cootype = scipy.genfromtxt(
            lib_end_points_filen,
            usecols=(1 - 1, 2 - 1, 7 - 1),
            unpack=True,
            dtype="S",
        )
        cootype = [str(c) for c in cootype]

        for i in range(len(lono)):
            # Get great-circle lons,lats given end-points
            if verbose:
                print("Reading pair-list for %s" % (name[i]))
            azs, lats, azcenter, latcenter = gcutils.get_gc_for_pair(
                lono[i],
                lato[i],
                lonf[i],
                latf[i],
            ...
            if ro[i] == rf[i]:
                D = ro[i] * np.ones_like(azs)
            elif rf[i] > ro[i] > 0.0:
                D = scipy.interp(azs, [lono[i], lonf[i]], [ro[i], rf[i]])
            else:
                D = -1 * np.ones_like(azs)

            # Create footprint appropriately depending on coord type, this will define the l,b,ra,dec attributes appropriately
            footprint = Footprint(
                azs, lats, name[i], Dist=D, degree=True, cootype=cootype[i]
            )
    ...
    def init_by_poles(self, gcstep=0.1, verbose=False):
        ...
        lib_poles_filen = os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            "lib",
            "lib_by_pole.dat",
        )

        name, pole_coot, c_coot = scipy.genfromtxt(
            lib_poles_filen, usecols=(0, 3, 6), unpack=True, dtype="S"
        )
        pole_coot = [str(c) for c in pole_coot]
        c_coot = [str(c) for c in c_coot]

        pole_dat = scipy.genfromtxt(
            lib_poles_filen,
            usecols=(1, 2, 4, 5, 7, 8, 9, 10),
        ...
        for i in np.arange(len(pole_lons)):
            ...
            if verbose:
                print("Reading pole-list for %s" % (name[i]))

            # If center coords are given, make sure they're in the same system as pole coords
            if clons[i] != -999.0 and clats[i] != -999.0:
                if c_coot[i] in pole_coot[i]:
                    center = [
                        clons[i],
                        clats[i],
                    ]  # If in the same system, just save
                    if verbose:
                        print(" same system, center:", center)
                else:
                    if "gal" in c_coot[i]:
                        clons[i], clats[i] = bovyc.lb_to_radec(
                            clons[i], clats[i], degree=True
                        )
                    else:
                        clons[i], clats[i] = bovyc.radec_to_lb(
                            clons[i], clats[i], degree=True
                        )
                    center = [clons[i], clats[i]]
                    if verbose:
                        print("computed center", center)
            else:
                center = None
            ...
            gc_lons, gc_lats = gcutils.get_gc_for_pole(
                pole_lons[i],
                pole_lats[i],
                degree=True,
                center=center,
            ...
            # Do linear interpolation for the distance, not much better to do
            if ros[i] == rfs[i]:
                D = ros[i] * np.ones_like(gc_lons)
            elif rfs[i] > ros[i] > 0.0:
                D = scipy.interp(gc_lons, [lono[i], lonf[i]], [ros[i], rfs[i]])
            else:
                D = -1 * np.ones_like(gc_lons)
    ...
    # -----------------by lonlat range--------------------------------------------------------------
    def init_by_lonlat_range(
        self, libname=None, Nran=1000, verbose=False, Rstat="mean"
    ):

        if libname is None:
            lib_llrange_filen = os.path.join(
                os.path.dirname(os.path.realpath(__file__)),
                "lib",
                "lib_by_lonlat_range.dat",
            )
        else:
            lib_llrange_filen = libname

        # Validate options for Rstat
        stat_types = ["min", "max", "mean", "median"]
        if Rstat not in stat_types:
            raise ValueError(
                "Invalid stat type. Expected one of: %s" % stat_types
            )
        Rstat_func = getattr(np, Rstat)

        name, cootype = scipy.genfromtxt(
            lib_llrange_filen, usecols=(0, 7), unpack=True, dtype="S"
        )
        cootype = [str(c) for c in cootype]

        azo_l, azf_l, lato_l, latf_l, ro_l, rf_l, stype = scipy.genfromtxt(
            lib_llrange_filen,
        ...
        for i in ind:
            # Parse corners
            if verbose:
                print("Reading lon-lat range for %s" % (name[i]))
            azo, azf, lato, latf, ro, rf = (
                azo_l[i],
                azf_l[i],
                lato_l[i],
                latf_l[i],
            ...
            Rs, azs, lats = get_random_spherical_coords(
                Nran,
                rad=[ro, rf],
                az=[azo, azf],
                lat=[lato, latf],
            ...
            footprint = Footprint(
                ...
                lats,
                name[i],
                Dist=Rstat_func(Rs) * np.ones_like(Rs),
                degree=True,
                cootype=cootype[i],
            )
    ...
    def init_by_star_list(self, verbose=False, Nint=300):
        ...
        list_dat = scipy.genfromtxt(
            lib_by_stars_list_filen,
            usecols=(1 - 1, 2 - 1, 3 - 1, 4 - 1, 5 - 1, 6 - 1),
        )
        list_sdat = scipy.genfromtxt(
            lib_by_stars_list_filen, usecols=(7 - 1, 8 - 1, 9 - 1), dtype="S"
        )

        # Deal with one-liners
        if np.ndim(list_dat) == 1:
            list_dat, list_sdat = (
                np.reshape(list_dat, (1, list_dat.size)),
                np.reshape(list_sdat, (1, list_sdat.size)),
            )
        ...
        (
            ...
            progtype_col,
            cntflg_col,
        ) = list_dat.astype(int).T
        # Convert col numbers to python-style
        lon_col, lat_col, Rhel_col, progtype_col, cntflg_col = (
            lon_col - 1,
            lat_col - 1,
            Rhel_col - 1,
            progtype_col - 1,
        ...
        for i in range(list_dat[:, 0].size):
            ...
            # print(lib_path, fname[i])

            if verbose:
                print("Reading star list for %s" % (name[i]))
            azs, lats, Rhels, cntflg = scipy.genfromtxt(
                os.path.join(lib_path, fname[i].decode("UTF-8")),
                usecols=(lon_col[i], lat_col[i], Rhel_col[i], cntflg_col[i]),
                unpack=True,
            )
            ...
            if cntflg_col[i] >= 0:
                if (cntflg == 1).sum() > 1:
                    print(
                        "Warning, more than one point flaged as center in %s, choosing first one."
                        % (fname[i])
                    )
                caz, clat = azs[cntflg == 1][0], lats[cntflg == 1][0]
                auxfoot = Footprint(
                    caz, clat, "dummy", degree=True, cootype=cootype[i]
                )
    ...
    def load_user_defined_centers_and_shortnames(self):
        ...
        lib_log_filen = os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            "lib",
            "lib_centers.log",
        )
        names, shortnames = scipy.genfromtxt(
            lib_log_filen, usecols=(0, 1), unpack=True, dtype="S"
        )
        _ra, _dec, _ll, _bb, _phi, _theta = scipy.genfromtxt(
            lib_log_filen,
            usecols=(2, 3, 4, 5, 6, 7),
            unpack=True,
            dtype=np.float,
        ...
        for ii in range(names.size):
            try:
                if not np.isnan(_ra[ii]) and not np.isnan(_dec[ii]):
                    self[names[ii]].cra, self[names[ii]].cdec = (
                        _ra[ii],
                        _dec[ii],
                    )
                if not np.isnan(_ll[ii]) and not np.isnan(_bb[ii]):
                    self[names[ii]].cl, self[names[ii]].cb = _ll[ii], _bb[ii]
                if hasattr(self[names[ii]], "cphi"):
                    if not np.isnan(_phi[ii]) and not np.isnan(_theta[ii]):
                        self[names[ii]].cphi, self[names[ii]].ctheta = (
                            _phi[ii],
                            _theta[ii],
                        )
                # Setting short names
                ...
                print(
                    "WARNING: Name %s used lib_centers.log not found in lib* definition files"
                    % (names[ii])
                )
    ...
    def __init__(self, verbose=True, gcstep=0.1, N=1000, Rstat="mean"):
        ...
        for i in self.keys():
            ...
            elif (self[i].Rhel < 0.0).any():
                self[i].Rgal[self[i].Rhel < 0.0] = np.nan
                self[i].phi[self[i].Rhel < 0.0] = np.nan
                self[i].theta[self[i].Rhel < 0.0] = np.nan
    ...
    def plot_stream_compilation(
        ...
        if cootype not in coo_types:
            raise ValueError(
                "Invalid coo type. Expected one of: %s" % coo_types
            )
        ...
        if plot_stream_type not in stream_types:
            raise ValueError(
                "Invalid stream type. Expected one of: %s" % stream_types
            )
        ...
        if "GC" in cootype:
            for i in self.keys():
                Rmax = np.max([Rmax, np.max(self[i].Rgal)])
        else:
            for i in self.keys():
                Rmax = np.max([Rmax, np.max(self[i].Rhel)])
        scatter_kwargs = dict(
            marker="o", s=8.0, edgecolor="none", vmin=0.0, vmax=Rmax, alpha=0.5
        )
        textlab_kwargs = dict(
            horizontalalignment="center", verticalalignment="bottom", zorder=99
        )
        textsym_kwargs = dict(
            marker="+", color="k", ms=5, zorder=textlab_kwargs["zorder"]
        )
        ...
        if Rrange[1] < 9e9:
            ...
            colorbar_kwargs = dict(extend="min")
        ...
        if text_kwargs is not None:
            for key in text_kwargs.keys():
                textlab_kwargs[key] = text_kwargs[key]
        if sym_kwargs is not None:
            for key in sym_kwargs.keys():
                textsym_kwargs[key] = sym_kwargs[key]
        if cb_kwargs is not None:
            for key in cb_kwargs.keys():
                colorbar_kwargs[key] = cb_kwargs[key]

        # If Rrange is supplied, use it to set vmax
        if Rrange[1] < 9e9:
            scatter_kwargs["vmax"] = Rrange[1]
        ...
        for i in keys_to_plot:
            ...
            if "GC" in cootype and self[i].phi is None:
                print(
                    "Skipping stream %s, no valid Rhel => no valid galactocentric attributes"
                    % (self[i].name)
                )
                continue
            ...
            if "GC" in cootype:
                notnan = (
                    self[i].Rgal >= 0.0
                )  # this works to avoid nans as well
                ro, rf, Rs = (
                    np.min(self[i].Rgal[notnan]),
                    np.max(self[i].Rgal[notnan]),
                    self[i].Rgal,
                )
            else:
                notneg = self[i].Rhel >= 0.0
                if notneg.any():
                    ro, rf, Rs = (
                        np.min(self[i].Rhel[notneg]),
                        np.max(self[i].Rhel[notneg]),
                        self[i].Rhel,
                    )
                else:
                    ro, rf, Rs = -1, -1, self[i].Rhel

            if "gal" in cootype:
                lons, latts, loncenter, latcenter = (
                    self[i].l,
                    self[i].b,
                    self[i].cl,
                    self[i].cb,
            ...
            if (ro > 0) and not (ro <= Rrange[1] and rf >= Rrange[0]):
                if verbose:
                    print(
                        "Skipping %s [%.1f,%.1f], outside selected Rrange [%.1f,%.1f]"
                        % (self[i].name, ro, rf, Rrange[0], Rrange[1],)
                    )
                continue
            else:
                if verbose and ro is not None:
                    print(
                        "Plotting %s [%.1f,%.1f], INside selected Rrange [%.1f,%.1f]"
                        % (self[i].name, ro, rf, Rrange[0], Rrange[1],)
                    )
            ...
            if plot_names:
                if use_shortnames:
                    plotname = self[i].sname
                else:
                    plotname = self[i].name
            ...
            if cc and plot_colorbar:
                ...
                cbar.solids.set_rasterized(
                    True
                )  # To avoid white lines in colorbar that appear some times
                cbar.solids.set_edgecolor("face")
                plot_colorbar = False


def plot_globular_clusters(
    ax, plot_colorbar=False, scat_kwargs=None, galactic=True
):
